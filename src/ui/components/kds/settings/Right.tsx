...
const Right = ({ settings }: any) => {
  ...
  return (
    <div className=" " style={{ backgroundColor: settings.pageBgColor }}>
      <div className="p-10">
            ...
            >
              ...
              {mockTicket.items.map((item) => (
                <div
                  ...
                  style={{
                    backgroundColor: item.status === 'completed' ? settings.itemCompletedBg : settings.itemPendingBg,
                    borderColor: item.status === 'completed' ? settings.itemCompletedBorder : settings.itemPendingBorder,
                    color: item.status === 'completed' ? settings.itemCompletedText : settings.itemPendingText,
              ...
              {settings.showPreparationTime && (
                <div className="text-center pt-2">
                  <p className="text-sm font-medium">
                    Preparation time: <span className="font-bold" style={{ color: settings.elapsedColor0to5 }}>{mockTicket.preparationTime}</span>
                  </p>
                </div>
              )}

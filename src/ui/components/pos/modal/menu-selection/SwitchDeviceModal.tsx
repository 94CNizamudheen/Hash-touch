...
const DEVICE_ROLES: { key: DeviceRole; label: string; icon: React.ReactNode }[] = [
  {
    key: "POS",
    label: "Point of Sale",
...
const SwitchDeviceModal = ({
  ...
  const availableRoles = DEVICE_ROLES.filter((r) => r.key !== currentRole);
  ...
  const handleSwitch = () => {
    ...
    showNotification.info(`${t("Switching to")} ${role.label}...`, 2000);
    ...
    <Sheet open={isOpen} onOpenChange={onClose}>
      ...
      >
        ...
        <div className="h-[calc(100%-140px)] sm:h-[80%] mt-6 sm:mt-10 px-4 sm:px-5 overflow-y-auto">
          ...
          {/* {currentRole && (
            <div className="mb-6 p-3 bg-primary rounded-lg border border-primary/20">
              <p className="text-xs text-background mb-1">{t("Current Mode")}</p>
              <p className="font-semibold text-secondary">
                {DEVICE_ROLES.find((r) => r.key === currentRole)?.label || currentRole}
              </p>
            </div>
          )} */}
